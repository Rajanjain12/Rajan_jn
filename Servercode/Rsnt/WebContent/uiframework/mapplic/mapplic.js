	...
	var Mapplic = function() {
		...
		function DevTools() {
			...
			this.init = function() {
				...
				$('.mapplic-layer', self.map).on('mouseup', function(e) {
		...
		var processData = function(data) {
			...
			self.levelselect = $("<select id='markers'></select>").addClass('mapplic-levels-select');
					...
					$.each(value.locations, function(index, value) {
						...
						if (value.pin != 'hidden') {
							if (self.o.locations) {
								...
								var pin = $('<div></div>').attr('id','btn-'+value.id).addClass('mapplic-pin').css({'top': top + '%', 'left': left + '%'}).appendTo(layer);
								var  spanpin = $('<span></span>').attr('id','span-'+value.id).addClass('large').text('Table-'+parseInt(index+1)).appendTo(pin);
		...
		function PinDrag() {
			this.init = function() {
				...
				/*	$(document).on('mouseup', function() {
						...
						$(document).off('mouseup');
							...
							var value = pin.data();
	                        console.log('value for dragged locations',value);
							value.x = x;
							value.y = y;
			...
			this.update = function(value) {
				$('#landmark-settings .title-input').val(value.title);
				...
				$('#landmark-settings .fill-input').iris('color', value.fill);
				$('#landmark-settings .link-input').val(value.link);

				if (value.category) $('#landmark-settings .category-select').val(value.category);
				else $('#landmark-settings .category-select').val('false');

				if (value.action) $('#landmark-settings .action-select').val(value.action);
				...
				$('#landmark-settings .thumbnail-input').val(value.thumbnail);
