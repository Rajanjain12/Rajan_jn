 ...
 * <i>jQuery.fn.Editor</i> as shown in the examples below.
 *
 *  @class
 ...
 *    for Editor. If used, the TableTools should be loaded first).
 * 
 *  @example
...
Editor.models.field = {
	 ...
	 * exactly the same way that DataTables using mDataProp for columns. This should be 
	 ...
	 * properties, in exactly the same way that DataTables using mDataProp for columns.
...
Editor.prototype.add = function ( field )
{
	...
	var template = $(
		'<div class="'+classesField.wrapper+' '+classesField.typePrefix+field.type+' '+classesField.namePrefix+field.name+'">'+
...
Editor.prototype.create = function ( title, buttons, show )
{
	...
	if ( show === undefined || show ) {
		this._display('open', function () {
			$('input:visible,select:visible,textarea:visible', that.dom.wrapper).filter(':eq(0)').focus();
		} );
	}
};
 ...
 *  @param {string} [name] The field name (from the <i>name</i> parameter given 
...
Editor.prototype._callbackFire = function ( trigger, args )
{
	...
	// Allow an array to be passed in for thr trigger to fire multiple events
	if ( $.isArray( trigger ) ) {
			...
			if ( json.error !== "" || json.fieldErrors.length !== 0 ) {
				...
				if ( json.fieldErrors.length > 0 ) {
					$(that.dom.bodyContent, that.s.wrapper).animate( {
						"scrollTop": $('div.'+that.classes.field.error+':eq(0)').position().top
					}, 600 );
				}
